...
public class DonHangDAOImpl implements DonHangDAO {
	...
	public List<DonHang> getAllDonHang() {
		// TODO Auto-generated method stub
		Session session = this.sessionFactory.getCurrentSession();
		List<DonHang> donHangs = session.createNativeQuery("select	maDonHang,ngayLap,MAKHACHHANG from DonHang ", DonHang.class).getResultList();
		return donHangs;
	}
	...
	public List<DonHang> getAllDonHangbyMaKhachHang(int maKhachHang) {
		...
		List<DonHang> donHangs = session
				.createNativeQuery("select * from DonHang d where d.MAKHACHHANG=" + maKhachHang + "", DonHang.class)
				.getResultList();
		return donHangs;
	}
}
