...
import vn.edu.iuh.cartbeans.Item;
import vn.edu.iuh.model.ChiTietDonHang;
import vn.edu.iuh.model.ChiTietDonHangID;
...
public class MuaHangController {
	...
	public String thanhToan(@RequestParam("tenTaiKhoan") String tenTaiKhoan, Model model)
	...
	public String xemThongTinSanPham(HttpServletRequest request, HttpServletResponse response, Model sanPhamModel,
			Model soLuongDaBan, Model soLuongCon) {
			...
			if (objGioHang != null) {
				...
				for (Item item : gioHang.getGioHang()) {
					ChiTietDonHang chiTietDonHang = new ChiTietDonHang(
							new ChiTietDonHangID(donHangService.getIdMax(),
									sanPhamService.getSanPhamByID(item.getId()).getMaSanPham()),
							sanPhamService.getSanPhamByID(item.getId()), donHang, item.getSoLuong(),
