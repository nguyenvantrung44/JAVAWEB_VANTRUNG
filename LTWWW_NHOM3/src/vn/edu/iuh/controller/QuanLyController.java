...
public class QuanLyController {
	...
	public String TrangQuanLy(HttpServletRequest request, HttpServletResponse response, Model model)
	...
	public String DSChiTiet(HttpServletRequest request, HttpServletResponse response, Model model)
			throws IOException, ServletException {
		HttpSession session = request.getSession();
		String id = request.getParameter("maDonHang");
		session.setAttribute("listChiTiet", chiTietDonHangService.getAllChiTietDonHangbyDonHang(Integer.parseInt(id)));
		return "dsDonHang_ChiTietQuanLy";
	}

	@RequestMapping("/dsSanPham_QuanLy")
	public String dsSanPham(HttpServletRequest request, HttpServletResponse response, Model model)
			throws IOException, ServletException {
		HttpSession session = request.getSession();
	...
	public String xoaSanPham(HttpServletRequest request, HttpServletResponse response, Model model)
			throws IOException, ServletException {
		HttpSession session = request.getSession();
	...
	public String formSuaSanPham(HttpServletRequest request, HttpServletResponse response, Model model)
			throws IOException, ServletException {
		HttpSession session = request.getSession();
	...
	public String themSanPham(HttpServletRequest request, HttpServletResponse response,
			@Valid @ModelAttribute("sanPhamThem") SanPham s, BindingResult result, Model model)
			throws IOException, ServletException {
		if (result.hasErrors()) {
			request.setAttribute("listDanhMuc", danhMucService.getAllDanhMuc());
			request.setAttribute("listNCC", nhaCungCapService.getAllNhaCungCap());
			...
			return "redirect:/dsSanPham_QuanLy";
		}
	}

	@RequestMapping(value = "/suaSanPham")
	public String suaSanPham(HttpServletRequest request, HttpServletResponse response, Model model)
			throws IOException, ServletException {
	...
	public String capNhatChiTiet(HttpServletRequest request) throws IOException, ServletException {
		...
		ChiTietDonHang ct = chiTietDonHangService.getChiTietbyDHSP(Integer.parseUnsignedInt(maDonHang),
